<?php
	...
	$contractAddr = $arrayBody["contractAddr"];
	...
	$sqlCount = "select count(*) count from contractList";

	if($createAddr != "")
	{
		$sql .= " where createAddr = \"" . $createAddr."\"";
		$sqlCount .= " where createAddr = \"" . $createAddr."\"";
	}
	else if($contractAddr != "")
	{
		$sql .= " where contractAddr = \"" . $contractAddr."\"";
		$sqlCount .= " where contractAddr = \"" . $contractAddr."\"";
	}
	else
	{
	...
	if($reslut)
	{
		...
		while($attr = $reslut->fetch_row())
		{
			...
			$returnBody[$i]["txHash"] = $attr[4];
			$returnBody[$i]["currentCoin"] = $attr[5];
			$returnBody[$i]["historyCoin"] = $attr[6];
			$returnBody[$i]["time"] = $attr[7];
